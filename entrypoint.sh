...
echo "${BARMAN_CRON_SCHEDULE} barman replication-status ${POSTGRES_SERVER} --minimal --target=wal-streamer | grep barman || barman /usr/local/bin/barman receive-wal --create-slot ${POSTGRES_SERVER}; /usr/local/bin/barman cron" >>/etc/cron.d/barman
